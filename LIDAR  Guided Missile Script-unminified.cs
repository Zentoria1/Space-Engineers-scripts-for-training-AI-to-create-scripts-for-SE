...
// Connector Types: ConnectorType.None, ConnectorType.Merge, ConnectorType.Connector, or ConnectorType.Rotor
// Use None type if you're planning to use your own disconnect mechanism
ConnectorType connectorType = ConnectorType.Merge;

// Missile Types: MissileType.Kinetic, or MissileType.Explosive (if using warheads)
MissileType missileType = MissileType.Kinetic;

// If you wish to fire missiles in sequence, add the tag below to the CUSTOM DATA of the missile's PB
// NOTE: (DO NOT add it to the custom name!)
...
// Should the missile self-destruct if it becomes idle after launching, ie loses its target (requires warheads)
// NOTE: By default, missiles will stop themselves and then shut off the PB to preserve runtime usage, but you can have them explode instead
bool selfDestructWhenIdle = false;

// How many ticks the missile should wait (once fired) before turning towards target (60 = 1 second). Default = 60.
// The distance from the center of the firing platform will also be calculated to ensure the missile is clear of the platform before turning
// NOTE: whichever comes last between clearanceTicks and clearanceMeters will be used.
int clearanceTicks = 60;

// How much power the missile should use during the initial launch procedure (while gaining clearance)
// Between 0 and 1.0f (0 - 100%). 'Atmo' is for atmospheric missiles.
// NOTE: Setting this to 0 means the thruster WILL NOT fire until the missile has cleared the platform's bounding box
//            If you're going to set it to 0, make sure you have some other means of getting it clear (ie side thruster fired via launch timer)
float clearancePowerSpace = 1.0f;
...
// If you want to want to have the script "trigger" timers at specified distances from the target, add the tag below to the
// CUSTOM DATA or CUSTOM NAME of as many timer blocks as you want, followed by the distance at which to start the timer
// EXAMPLE: [T:100] => this timer will start when the missile comes within 100m of the target
const string timerTag = "[T:";

// If you want to have the script trigger a LAUNCH timer BEFORE launching the missile, enter the EXACT NAME here
// This can be used in conjunction with Auto Launch to perform other functions per missile when multiple are set to launch
// NOTE: Because this is done before the missile is diconnected from the platform, each missile MUST have a separate launch timer
//            and every launch timer MUST have a unique name
const string launchTimerName = "Missle Launch Timer";
...
const double derivativeConstant = 3;   // derivative gain of gyroscopes
...
const string DEBUG_LCD = "Missile 8 Debug LCD";
...
/* TODO:
  * Modded missiles are missing their targets??? try to find out why! --> seems to be the speed increase?? Might not have enough time to max out speed?
*/
...
StringBuilder _timerSB = new StringBuilder();
...
public void SubMain(string argument, UpdateType updateSource)
{
    ...
    if (killSignalRcvd)
    {
        if (selfDestructWhenIdle && _warheads.Count > 0)
            KillGuidance(true);
        else if (_missileInfo.Speed > 1)
    ...
    else if (_conBlock?.Enabled != false && argument.Length == 0 && !msgPending && !startLaunchSequence && !readyToLaunch && !inFlight && !sendRollSpeedInfo && !sendSpiralInfo && !updateRollAndSpiral)
    {
    ...
    if (startLaunchSequence)
    {
        ...
        if (GridTerminalSystem.GetBlockWithId(_projector.EntityId) != null)
        {
            if (DEBUG)
            {
    ...
    if (tryStartSpiral || activateRoll)
    {
        var distanceCheck = Vector3D.DistanceSquared(_missileInfo.Position, _target.Position) < distanceBeforeSpiral * distanceBeforeSpiral;

        if (tryStartSpiral)
        {
            activateSpiral = distanceCheck;
            tryStartSpiral = !activateSpiral;
        }

        if (activateRoll)
        {
            useRoll = distanceCheck;
            activateRoll = !useRoll;
        }
    }
...
void CheckTimers()
{
    var distanceToTgtSqd = Vector3D.DistanceSquared(_target.Position, _missileInfo.Position);
    ...
    for (int i = _timers.Count - 1; i >= 0; i--)
    {
        ...
        if (timer.Status == TimerStatus.Stopped && distanceToTgtSqd <= timer.DistanceSquared)
            timer.Trigger();
    }
}
...
void KillGuidance(bool destruct = false)
{
...
public void Intercept()
{
    ...
    {
        if (_missileInfo.Speed > 0 && headingVec.Length() / _missileInfo.Speed <= 2)
            activateSpiral = false;
        else
        {
...
public Vector3D CalculateHeadingVec(Vector3D myVelocity, Vector3D interceptVec)
{
    ...
    double altitude = _missileInfo.Altitude;
    if (altitude > 0)
    {
        if (DEBUG)
...
new RayD(pos, myVelocity)
    ...
    if (myVelocity.LengthSquared() < 1000 || myVelocity.Dot(interceptVec) < 0)
    {
        if (DEBUG)
            _debug.Append($"Returning interceptVec from CalculateHeadingVec\n");

        return interceptVec;
    }

    if (DEBUG)
        _debug.Append($"Returning ReflectVector\n");
...
bool InitialSetup()
{
    ...
    clearancePowerSpace = MathHelper.Clamp(clearancePowerSpace, 0f, 1.0f);
    clearancePowerAtmo = MathHelper.Clamp(clearancePowerAtmo, 0f, 1.0f);
    ...
    bool conIssue = _conBlock == null && connectorType != ConnectorType.None;
    if (_projector == null || conIssue)
    {
        ...
        if (conIssue)
            Echo("* No connector/merge/rotor\n   block found! If this\n   is intended, set the\n   connectorType to NONE!");
        return false;
    }

    var lTimer = GridTerminalSystem.GetBlockWithName(launchTimerName) as IMyTimerBlock;
    if (lTimer != null)
        _launchTimer = new Timer(lTimer, true);
...
void CheckMissile()
{
    ...
    HashSet<ThrusterType> thrustTypes = new HashSet<ThrusterType>();

    int numWarheads = 0, numGyros = 0;
    float totalPowerConsumption = 0;
    double totalPowerProduced = 0;
    bool needsConnector = connectorType != ConnectorType.None;
    bool hasRemote, hasAntenna, hasGyro, hasConnector, hasEnoughPower, hasEnoughThrust;
    hasRemote = hasAntenna = hasConnector = hasGyro = hasEnoughPower = hasEnoughThrust = false;
    IMyShipController shipController = null;

    foreach (var block in tempList)
    {
        int num;
        _blockNumbersDict.TryGetValue(block.BlockDefinition, out num);
        _blockNumbersDict[block.BlockDefinition] = num + 1;

        var powerBlock = block as IMyPowerProducer;
        if (powerBlock == null && !_requiredInputDict.ContainsKey(block.BlockDefinition))
            GetRequiredPowerInput(block);

        if (powerBlock != null)
        {
            totalPowerProduced += GetPowerGeneratedByBlock(block);
            continue;
        }

        if (block is IMyShipConnector || block is IMyMotorStator || block is IMyShipMergeBlock)
        {
            hasConnector = true;
            continue;
        }
    ...
    double neededThrust = 1, totalThrust = 0;
    ...
    if (shipController != null)
    {
        ...
        {
            var t = _thrusters[i];
            if (Base6Directions.GetOppositeDirection(t.Orientation.Forward) != shipController.Orientation.Forward)
        ...
        foreach (var t in _thrusters)
        {
            ...
            totalThrust += t.MaxEffectiveThrust;

            var thrustType = GetThrusterType(t, gravity);
            if (thrustType != ThrusterType.None)
                thrustTypes.Add(thrustType);
        }

        if (gravity.LengthSquared() > 0)
        {
            var mass = shipController.CalculateShipMass().PhysicalMass;
            neededThrust = mass * gravity.Length();
        }

        hasEnoughThrust = neededThrust / totalThrust < 0.8f;
    }

    foreach (var kvp in _requiredInputDict)
    {
        int num;
        _blockNumbersDict.TryGetValue(kvp.Key, out num);

        totalPowerConsumption += num * kvp.Value;
    }

    hasEnoughPower = totalPowerConsumption < totalPowerProduced;

    _debug.Clear()
  .Append("Missile Check Status\n--------------------\n")
  .Append("Remote Status: ")
  .Append(hasRemote ? "OK" : "MISSING")
  .Append("\nAntenna Status: ")
  .Append(hasAntenna ? "OK" : "MISSING")
  .Append("\nGyro Status: ")
  .Append(hasGyro ? "OK" : "MISSING")
  .Append("\nConnector Status: ")
  .Append(hasConnector || !needsConnector ? "OK" : "MISSING")
  .Append("\nThrust Ratio Status: ")
  .Append(hasEnoughThrust ? "OK" : "NOT ENOUGH FORWARD THRUST")
  .Append("\nPower Ratio Status: ")
  .Append(hasEnoughPower ? "OK" : "NOT ENOUGH POWER")
  .Append("\n\nDetails\n--------------------\n")
  .Append("Connector Type Setting: ")
  .Append(connectorType)
  .Append(" (configured in top portion of script)\nThruster Type Setting: ")
  .Append(thrusterType)
  .Append(" (configured in top portion of script)\nThruster Types Found: ");

    if (thrustTypes.Count > 0)
    {
        foreach (var thrustType in thrustTypes)
            _debug.Append(thrustType)
              .Append(", ");
    ...
    _debug.Append("\nMissile Mass: ")
  .Append(hasRemote ? shipController.CalculateShipMass().PhysicalMass.ToString("0") + " Kg" : "NEED REMOTE TO CALCULATE")
  .Append("\nThrust Needed: ")
  .Append(neededThrust.ToString("0"))
  .Append(" N\nThrust Available: ")
  .Append(totalThrust.ToString("0"))
  .Append(" N\nPower Consumed: ")
  .Append(totalPowerConsumption.ToString("0"))
  .Append(" W\nPower Produced: ")
  .Append(totalPowerProduced.ToString("0"))
  .Append(" W\nForward Thrusters Found: ")
  .Append(hasRemote ? _thrusters.Count : 0)
  .Append("\nGyros Found: ")
  .Append(numGyros)
  .Append("\nWarheads Found: ")
  .Append(numWarheads);

    Echo($"Missile Check details written\nto Custom Data!");
    Me.CustomData = _debug.ToString();

    _debug.Clear();
    _thrusters.Clear();
    tempList.Clear();
    _requiredInputDict.Clear();
    _blockNumbersDict.Clear();
}

MyDefinitionId ElectricityId = MyDefinitionId.Parse("MyObjectBuilder_GasProperties/Electricity");
Dictionary<MyDefinitionId, float> _requiredInputDict = new Dictionary<MyDefinitionId, float>();
Dictionary<MyDefinitionId, int> _blockNumbersDict = new Dictionary<MyDefinitionId, int>();
Dictionary<MyDefinitionId, float> _thrusterPowerInputDict = new Dictionary<MyDefinitionId, float>()
    {
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockSmallThrust"), 3.36f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockLargeThrust"), 33.6f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockLargeHydrogenThrust"), 7.5f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockSmallHydrogenThrust"), 1.25f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockLargeAtmosphericThrust"), 16.8f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockSmallAtmosphericThrust"), 2.4f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockSmallThrustSciFi"), 3.36f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockLargeThrustSciFi"), 33.6f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockLargeAtmosphericThrustSciFi"), 16.8f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/LargeBlockSmallAtmosphericThrustSciFi"), 2.4f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockSmallThrust"), 0.2f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockLargeThrust"), 2.4f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockLargeHydrogenThrust"), 0.6f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockSmallHydrogenThrust"), 0.125f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockLargeAtmosphericThrust"), 2.4f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockSmallAtmosphericThrust"), 0.6f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockSmallThrustSciFi"), 0.2f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockLargeThrustSciFi"), 2.4f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockLargeAtmosphericThrustSciFi"), 2.4f },
      { MyDefinitionId.Parse("MyObjectBuilder_Thrust/SmallBlockSmallAtmosphericThrustSciFi"), 0.6f },
    };

void GetRequiredPowerInput(IMyTerminalBlock block)
{
    float input = 0;
    if (block is IMyThrust && _thrusterPowerInputDict.TryGetValue(block.BlockDefinition, out input))
    {
        _requiredInputDict[block.BlockDefinition] = (float)Math.Round(input * 1000000, MidpointRounding.AwayFromZero);
        return;
    }

    if (block is IMyRadioAntenna)
    {
        _requiredInputDict[block.BlockDefinition] = 50000; // setting it to 50 kW since the max lidar range is 10 km - no reason to use max antenna power draw
        return;
    }

    MyResourceSinkComponent sinkComponent;
    if (block.Components.TryGet(out sinkComponent))
    {
        try
        {
            bool wasOff = false;
            ...
            if (funcBlock != null)
            {
                wasOff = !funcBlock.Enabled;
                funcBlock.Enabled = true;
            }

            var maxInput = Math.Round(sinkComponent.MaxRequiredInputByType(ElectricityId) * 1000000, MidpointRounding.AwayFromZero);
            input = (float)maxInput;
    ...
    if (input > 0)
        _requiredInputDict[block.BlockDefinition] = input;
}

double GetPowerGeneratedByBlock(IMyTerminalBlock block)
{
    MyResourceSourceComponent sourceComponent;
    double maxGen = 0;
    if (block.Components.TryGet(out sourceComponent))
    {
        try
        {
            bool wasOff = false;
            ...
            maxGen = sourceComponent.MaxOutputByType(ElectricityId);
    ...
    return (maxGen == 0) ? 0 : Math.Round(maxGen * 1000000, MidpointRounding.AwayFromZero);
}
...
bool Init()
{
    ...
    foreach (var b in tempList)
    {
        ...
        if (timer != null)
        {
            var t = new Timer(timer, false, timerTag, _timerSB);
            if (t.IsValid)
                _timers.Add(t);
...
[Flags]
enum ConnectorType { None = 0, Merge = 1, Connector = 2, Rotor = 4 }

MyDefinitionId HydroDef = new MyDefinitionId(typeof(MyObjectBuilder_GasProperties), "Hydrogen");

ThrusterType GetThrusterType(IMyThrust thruster, Vector3D gravity)
{
    var subtype = thruster.BlockDefinition.SubtypeName.Replace("SciFi", "");
    ...
    if (sink?.AcceptedResources.Contains(HydroDef) == true)
        return ThrusterType.Hydro;

    var thrustRatio = thruster.MaxEffectiveThrust / thruster.MaxThrust;
    var gravSquared = gravity.LengthSquared();
...
public class Target
{
    ...
    public void Update(long id, double updateTime, Vector3D velocity, Vector3D accel, Vector3D position)
    {
        ...
        if (id == TargetID)
        {
            ...
            {
                bool atMaxSpeed = velocity.LengthSquared() >= maxSpeed * maxSpeed;
                double newSpeed;

                if (atMaxSpeed)
                {
                    newSpeed = maxSpeed + 1;
                    velocity = Vector3D.Normalize(velocity) * newSpeed;
                }
                if (Vector3D.IsZero(velocity, 0.01))
                {
                    newSpeed = 0;
                ...
                if (newSpeed == 0 || atMaxSpeed || Math.Abs(newSpeed - _lastSpeed) < 0.5 || Vector3D.IsZero(accel, 0.01))
                {
                ...
                {
                    Acceleration = accel.Length() * Math.Sign(accel.Dot(velocity));
                    AccelVector = accel;
                }
    ...
    public Vector3D CalculateInterceptVector(WorldInfo wInfo, out double timeToTarget)
    {
        ...
        if (double.IsInfinity(timeToTarget))
        {
            if (DEBUG)
                _debug.Append($"Time is too big\nGoing direct\n");

            var velDot = wInfo.Velocity.Dot(Velocity);
            var addVec = (velDot < 0 || mySpeed == 0) ? Velocity * _run : Velocity * (displacementVector.Length() / mySpeed);

            InterceptVector = displacementVector + addVec;
        }
        else
...
enum TimerStatus { Started, Stopped }
...
class Timer
{
    ...
    public Timer(IMyTimerBlock timer, bool isLaunch, string tag = null, StringBuilder sb = null)
    {
        _timer = timer;
        Name = timer.CustomName;
        ...
        string name, toCheck;
        string cName = Name;
        string cData = timer.CustomData;

        if (cName.Contains(tag))
            name = cName;
        else if (cData.Contains(tag))
            name = cData;
        else
        {
            IsValid = false;
        ...
        toCheck = name.Substring(name.IndexOf(tag)).Split(tag.Last())[1].Trim();
        sb.Clear();
        ...
        timer.Enabled = true;
        IsValid = timer.IsWorking && int.TryParse(sb.ToString(), out DistanceSquared);
...
public class WorldInfo
{
    ...
    public void Update()
    {
        ...
        var atMaxSpeed = newVelocity.LengthSquared() >= maxSpeed * maxSpeed;

        //if (atMaxSpeed)
        //  newVelocity = Vector3D.Normalize(newVelocity) * maxSpeed + 1;

        if (atMaxSpeed || Vector3D.IsZero(newVelocity, 0.01) || Vector3D.IsZero(newVelocity - Velocity, 0.01))
        {
            AccelVector = Vector3D.Zero;
            Acceleration = 0;
        }
        ...
        if (Vector3D.IsZero(newVelocity, 0.01))
        {
            Velocity = Vector3D.Zero;
            Speed = 0;
        }
